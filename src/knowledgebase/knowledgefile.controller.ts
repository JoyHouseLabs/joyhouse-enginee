...
export class KnowledgefileController {
  ...
  async getAll(
    @Req() req,
    @Query() query: import('./knowledgefile-query.dto').KnowledgefileQueryDto,
  ) {
    const userId = req.user.sub;
    const { knowledgebaseId, page = 1, pageSize = 10 } = query;
    return this.kfService.findAll(
      knowledgebaseId ?? '',
      userId,
      page,
      pageSize,
  ...
  async create(
    @Body() body: import('./knowledgefile-create.dto').KnowledgefileCreateDto,
    @Req() req,
  ) {
    const userId = req.user.sub;
  ...
  async update(
    @Body() body: import('./knowledgefile-update.dto').KnowledgefileUpdateDto,
    @Req() req,
  ) {
    const userId = req.user.sub;
