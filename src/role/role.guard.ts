...
export class RoleGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    console.log('[RoleGuard] controllerName:', controllerName, 'methodName:', methodName);
    ...
    if (requiredRoles && requiredRoles.length) {
      ...
      // 数据库动态权限
      const allowed = await this.roleService.getAllowedRoles(controllerName, methodName);
      allowedRoles = allowed.map(r => r.name);
    }
    console.log('[RoleGuard] requiredRoles:', requiredRoles, 'allowedRoles:', allowedRoles);
    ...
    if (!userRoleNames.some(role => allowedRoles.includes(role))) {
      console.log('[RoleGuard] 没有权限, userRoleNames:', userRoleNames, 'allowedRoles:', allowedRoles);
