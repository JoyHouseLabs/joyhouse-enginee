...
export class WorkflowService {
  ...
  async execute(id: string, executeWorkflowDto: ExecuteWorkflowDto, user: User): Promise<WorkflowExecution> {
    const workflow = await this.findOne(id, user);
  ...
  async getExecutions(workflowId: string, user: User): Promise<WorkflowExecution[]> {
  ...
  async getExecution(executionId: string, user: User): Promise<WorkflowExecution> {
  ...
  async getExecutionSteps(executionId: string, user: User): Promise<WorkflowExecutionStep[]> {
  ...
  async continueExecution(executionId: string, continueDto: ContinueExecutionDto, user: User): Promise<void> {
    const execution = await this.getExecution(executionId, user);
    await this.workflowEngine.continueExecution(execution.id, continueDto.input);
  }

  async handleApproval(executionId: string, approvalDto: ApprovalDto, user: User): Promise<void> {
    const execution = await this.getExecution(executionId, user);
    await this.workflowEngine.handleApproval(execution.id, approvalDto.approved, approvalDto.comment);
  }

  async cancelExecution(executionId: string, user: User): Promise<WorkflowExecution> {
    const execution = await this.getExecution(executionId, user);
  ...
  async createFromTemplate(templateId: string, name: string, user: User): Promise<Workflow> {
  ...
  async validateWorkflow(id: string, user: User): Promise<{ valid: boolean; errors: string[] }> {
    const workflow = await this.findOne(id, user);
    const errors: string[] = [];
      ...
      switch (node.type) {
        ...
        case 'condition':
          if (!node.data.condition) {
            errors.push(`Condition node "${node.label}" must specify condition`);
          }
          break;
        case 'script':
          if (!node.data.script) {
        ...
        case 'delay':
          if (!node.data.delayMs || node.data.delayMs <= 0) {
            errors.push(`Delay node "${node.label}" must specify valid delayMs`);
          }
          break;
        case 'wait_event':
          if (!node.data.eventType) {
            errors.push(`Wait event node "${node.label}" must specify eventType`);
          }
          break;
        case 'loop_start':
          if (!node.data.loopId) {
          ...
          if (node.data.maxIterations && node.data.maxIterations <= 0) {
            errors.push(`Loop start node "${node.label}" maxIterations must be positive`);
          }
          break;
        case 'loop_end':
          if (!node.data.loopId) {
        ...
        case 'loop_condition':
          if (!node.data.loopId) {
            errors.push(`Loop condition node "${node.label}" must specify loopId`);
          }
          break;
        case 'parallel_start':
          if (!node.data.parallelId) {
            errors.push(`Parallel start node "${node.label}" must specify parallelId`);
          }
          if (node.data.parallelTimeout && node.data.parallelTimeout <= 0) {
            errors.push(`Parallel start node "${node.label}" parallelTimeout must be positive`);
          }
          break;
        case 'parallel_end':
          if (!node.data.parallelId) {
            errors.push(`Parallel end node "${node.label}" must specify parallelId`);
          }
          break;
        case 'parallel_branch':
          if (!node.data.parallelId) {
            errors.push(`Parallel branch node "${node.label}" must specify parallelId`);
          }
          if (!node.data.branchName) {
            errors.push(`Parallel branch node "${node.label}" must specify branchName`);
          }
          break;
      }
    });
    ...
    // 验证循环结构
    const loopValidation = this.validateLoopStructure(workflow.nodes, workflow.edges);
    errors.push(...loopValidation);

    // 验证并发结构
    const parallelValidation = this.validateParallelStructure(workflow.nodes, workflow.edges);
  ...
  private validateLoopStructure(nodes: any[], edges: any[]): string[] {
    ...
    const loopConditions = nodes.filter(node => node.type === 'loop_condition');

    // 检查每个loop_start都有对应的loop_end
    loopStarts.forEach(startNode => {
      const loopId = startNode.data.loopId;
      const matchingEnds = loopEnds.filter(endNode => endNode.data.loopId === loopId);
      
      if (matchingEnds.length === 0) {
        errors.push(`Loop start "${startNode.label}" (${loopId}) has no matching loop end`);
      } else if (matchingEnds.length > 1) {
        errors.push(`Loop start "${startNode.label}" (${loopId}) has multiple matching loop ends`);
      }
    });
    ...
    loopEnds.forEach(endNode => {
      ...
      const matchingStarts = loopStarts.filter(startNode => startNode.data.loopId === loopId);
      
      if (matchingStarts.length === 0) {
        errors.push(`Loop end "${endNode.label}" (${loopId}) has no matching loop start`);
      }
    });
    ...
    loopConditions.forEach(conditionNode => {
      ...
      if (matchingStarts.length === 0) {
        errors.push(`Loop condition "${conditionNode.label}" (${loopId}) has no matching loop start`);
      }
    });
    ...
    loopStarts.forEach(startNode => {
      ...
      if (endNode) {
        // 检查是否存在从start到end的路径
        const hasPath = this.hasPathBetweenNodes(startNode.id, endNode.id, nodes, edges);
        if (!hasPath) {
          errors.push(`No path found from loop start "${startNode.label}" to loop end "${endNode.label}"`);
        }
      }
    });
  ...
  private hasPathBetweenNodes(startId: string, endId: string, nodes: any[], edges: any[]): boolean {
  ...
  private validateParallelStructure(nodes: any[], edges: any[]): string[] {
    const errors: string[] = [];
    const parallelStarts = nodes.filter(node => node.type === 'parallel_start');
    const parallelEnds = nodes.filter(node => node.type === 'parallel_end');
    const parallelBranches = nodes.filter(node => node.type === 'parallel_branch');

    // 检查每个parallel_start都有对应的parallel_end
    parallelStarts.forEach(startNode => {
      const parallelId = startNode.data.parallelId;
      const matchingEnds = parallelEnds.filter(endNode => endNode.data.parallelId === parallelId);
      
      if (matchingEnds.length === 0) {
        errors.push(`Parallel start "${startNode.label}" (${parallelId}) has no matching parallel end`);
      } else if (matchingEnds.length > 1) {
        errors.push(`Parallel start "${startNode.label}" (${parallelId}) has multiple matching parallel ends`);
      }
    });
    ...
    parallelEnds.forEach(endNode => {
      ...
      const matchingStarts = parallelStarts.filter(startNode => startNode.data.parallelId === parallelId);
      
      if (matchingStarts.length === 0) {
        errors.push(`Parallel end "${endNode.label}" (${parallelId}) has no matching parallel start`);
      }
    });
    ...
    parallelBranches.forEach(branchNode => {
      ...
      if (matchingStarts.length === 0) {
        errors.push(`Parallel branch "${branchNode.label}" (${parallelId}) has no matching parallel start`);
      }
    });
    ...
    parallelStarts.forEach(startNode => {
      const parallelId = startNode.data.parallelId;
      const endNode = parallelEnds.find(node => node.data.parallelId === parallelId);
      
      if (endNode) {
        // 检查是否存在从start到end的路径
        const hasPath = this.hasPathBetweenNodes(startNode.id, endNode.id, nodes, edges);
        if (!hasPath) {
          errors.push(`No path found from parallel start "${startNode.label}" to parallel end "${endNode.label}"`);
        }
      }
    });
