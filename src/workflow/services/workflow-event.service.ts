...
export class WorkflowEventService {
  ...
  async handleWeatherUpdate(data: { location: string; temperature: number; condition: string; timestamp: Date }) {
    this.logger.log(`Weather updated for ${data.location}: ${data.condition}, ${data.temperature}°C`);
    await this.workflowEngine.handleEvent('weather_update', data);
  }

  @OnEvent('news.trending')
  async handleTrendingNews(data: { keywords: string[]; articles: any[]; timestamp: Date }) {
    this.logger.log(`Trending news keywords: ${data.keywords.join(', ')}`);
    await this.workflowEngine.handleEvent('trending_news', data);
  }

  @OnEvent('user.action')
  async handleUserAction(data: { userId: string; action: string; data: any; timestamp: Date }) {
  ...
  async handleSystemAlert(data: { level: 'info' | 'warning' | 'error'; message: string; source: string; timestamp: Date }) {
    this.logger.log(`System alert [${data.level}] from ${data.source}: ${data.message}`);
    await this.workflowEngine.handleEvent('system_alert', data);
  }

  @OnEvent('webhook.received')
  async handleWebhook(data: { url: string; method: string; headers: any; body: any; timestamp: Date }) {
  ...
  async handleScheduleTrigger(data: { scheduleId: string; cron: string; timestamp: Date }) {
