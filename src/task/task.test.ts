...
async function test() {
  ...
  try {
    ...
    // 保存任务组
    const savedTaskGroup = await dataSource
    ...
    // 保存关联任务项
    const savedGroupTask = await dataSource
      .getRepository(TaskItem)
      .save(groupTask);
    console.log('创建关联任务项成功:', savedGroupTask);

    // 4. 查询任务组及其任务项
    const taskGroupWithItems = await dataSource
      .getRepository(TaskGroup)
      .findOne({
    ...
    console.log(
      '查询任务组及其任务项:',
      JSON.stringify(taskGroupWithItems, null, 2),
    );
