...
export class TaskController {
  ...
  create(@User('id') userId: string, @Body() dto: TaskCreateDto): Promise<TaskDto> {
  ...
  findAll(@User('id') userId: string, @Query() query: TaskQueryDto): Promise<PaginatedResponseDto<TaskDto>> {
  ...
  findOne(@User('id') userId: string, @Param('id') id: string): Promise<TaskDto> {
  ...
  @ApiResponse({ status: 200, type: TaskDto })
  update(@User('id') userId: string, @Param('id') id: string, @Body() dto: TaskUpdateDto): Promise<TaskDto> {
  ...
  @ApiResponse({ status: 200, type: TaskDto })
  startTask(@User('id') userId: string, @Param('id') id: string): Promise<TaskDto> {
    return this.taskService.updateTaskStatus(id, userId, TaskStatus.IN_PROGRESS);
  }

  @Post('end/:id')
  @ApiOperation({ summary: '结束任务' })
  @ApiResponse({ status: 200, type: TaskDto })
  endTask(@User('id') userId: string, @Param('id') id: string): Promise<TaskDto> {
  ...
  @ApiResponse({ status: 200, type: PaginatedResponseDto })
  getTodayTasks(@User('id') userId: string, @Query() query: TaskQueryDto): Promise<PaginatedResponseDto<TaskDto>> {
  ...
  @ApiResponse({ status: 200, type: PaginatedResponseDto })
  getWeekTasks(@User('id') userId: string, @Query() query: TaskQueryDto): Promise<PaginatedResponseDto<TaskDto>> {
  ...
  @ApiResponse({ status: 200, type: PaginatedResponseDto })
  getMonthTasks(@User('id') userId: string, @Query() query: TaskQueryDto): Promise<PaginatedResponseDto<TaskDto>> {
