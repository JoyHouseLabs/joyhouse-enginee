...
export class ReplyController {
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        list: [{ content: 'xxx', target: 'note', targetId: '1' }],
        total: 11,
        total_page: 2,
        pagesize: 10,
      },
  ...
  ): Promise<ReplyListPageDto> {
    const userId = req.user.sub;
    const { page = 1, pageSize = 10, target, targetId } = query;
    const { data, total } = await this.replyService.findAll(
      userId,
      Number(page),
      Number(pageSize),
      target,
  ...
  @ApiResponse({
    status: 200,
    type: ReplyListItemDto,
    description: '获取单条回复',
  })
  async getReply(
    @Param('id') id,
    @Req() req,
  ): Promise<ReplyListItemDto | null> {
    const userId = req.user.sub;
